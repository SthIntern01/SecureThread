    ...
    <div className="theme-card rounded-lg p-6">
      ...
      <div className="space-y-3">
        {owaspCategories.map((category) => {
          ...
          if (!analysisData) {
            return (
              <div key={category.id} className="theme-bg-subtle rounded-lg p-4 opacity-50">
                <div className="flex items-center justify-between">
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      <div className="theme-text font-medium">{category.name}</div>
                      <div className="theme-text-muted text-sm">{category.id}</div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="theme-text-muted text-sm">No data</div>
                  </div>
                </div>
              </div>
            );
          ...
          return (
            <div key={category.id} className="theme-bg-subtle rounded-lg p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-start space-x-3 flex-1">
                  <div className="theme-text-secondary font-mono text-sm mt-1 min-w-[60px]">
                    {category.id.split(':')[0]}
                  </div>
                  <div className="flex-1">
                    <div className="theme-text font-medium">{analysisData.name}</div>
                    <div className="theme-text-muted text-sm mb-2">{category.id}</div>
                ...
                <div className="text-right ml-4">
                  ...
                  <div className="theme-text-muted text-sm">Issues</div>
                </div>
              </div>
            </div>
          );
