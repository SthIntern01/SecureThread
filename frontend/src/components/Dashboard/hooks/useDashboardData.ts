...
export const useDashboardData = () => {
  ...
  // Fetch advanced metrics from new API
  // Replace the fetchAdvancedMetrics function with this version:
const fetchAdvancedMetrics = async (
  ...
  codeQualityMetrics: {
    ...
    // 🔧 FIX: Add total files from security metrics
    total_files: data.security_metrics?.total_files_scanned || data.code_quality_metrics?.total_files || 0
  },
  ...
  // 🔧 FIX: Get technical debt from the correct API location
  technicalDebtDetailed: data.code_quality_metrics?.technical_debt ? {
    ...
    debtBySeverity: {
      // Calculate from vulnerability data if available
      critical: {
  ...
  const filterByTimeRange = (date: string, timeFilter: TimeFilter): boolean => {
  ...
  const fetchUserRepositories = async (userId: number, timeFilter: TimeFilter, selectedRepository: number | 'all'): Promise<Repository[]> => {
    try {
      const token = localStorage.getItem("access_token");
      ...
      if (response.ok) {
        ...
        if (data.user_id && userInfo && data.user_id !== userInfo.id) {
  ...
  const fetchUserCustomScanResults = async (userId: number, timeFilter: TimeFilter, selectedRepository: number | 'all'): Promise<CustomScanResult[]> => {
    try {
      const token = localStorage.getItem("access_token");
