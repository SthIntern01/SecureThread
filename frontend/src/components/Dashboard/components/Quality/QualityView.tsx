...
import { Code, Wrench, Bug, TrendingUp, AlertCircle } from "lucide-react";
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className={`theme-card rounded-lg p-4 border-l-4 ${
          maintainabilityIndex > 75 ? 'border-green-500' :
          maintainabilityIndex > 50 ? 'border-yellow-500' :
          maintainabilityIndex > 25 ? 'border-orange-500' : 'border-red-500'
        }`}>
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs theme-text-muted uppercase tracking-wide">Maintainability</span>
          </div>
        ...
        <div className="theme-card rounded-lg p-4 border-l-4 border-yellow-500">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs theme-text-muted uppercase tracking-wide">Tech Debt</span>
          </div>
          <div className="text-3xl font-bold text-yellow-400">
            {totalDebtHours}h
          </div>
          <div className="theme-text-muted text-sm mt-1">Remediation effort</div>
        </div>

        {/* Code Smells Card */}
        <div className={`theme-card rounded-lg p-4 border-l-4 ${
          totalSmells === 0 ? 'border-green-500' : 
          totalSmells < 10 ? 'border-blue-500' : 
          totalSmells < 50 ? 'border-yellow-500' : 'border-red-500'
        }`}>
          <div className="flex items-center justify-between mb-2">
            <Bug className={`w-5 h-5 ${
              totalSmells === 0 ? 'text-green-400' : 
              totalSmells < 10 ? 'text-blue-400' : 
              totalSmells < 50 ? 'text-yellow-400' : 'text-red-400'
            }`} />
            <span className="text-xs theme-text-muted uppercase tracking-wide">Code Smells</span>
          </div>
          <div className={`text-3xl font-bold ${
            totalSmells === 0 ? 'text-green-400' : 
            totalSmells < 10 ? 'text-blue-400' : 
        ...
        {/* Complexity Card */}
        <div className={`theme-card rounded-lg p-4 border-l-4 ${
          complexityScore < 25 ? 'border-green-500' :
          complexityScore < 50 ? 'border-blue-500' :
          complexityScore < 75 ? 'border-orange-500' : 'border-red-500'
        }`}>
          <div className="flex items-center justify-between mb-2">
            <TrendingUp className={`w-5 h-5 ${
              complexityScore < 25 ? 'text-green-400' :
              complexityScore < 50 ? 'text-blue-400' :
              complexityScore < 75 ? 'text-orange-400' : 'text-red-400'
            }`} />
            <span className="text-xs theme-text-muted uppercase tracking-wide">Complexity</span>
          </div>
          <div className={`text-3xl font-bold ${
            complexityScore < 25 ? 'text-green-400' :
            complexityScore < 50 ? 'text-blue-400' :
