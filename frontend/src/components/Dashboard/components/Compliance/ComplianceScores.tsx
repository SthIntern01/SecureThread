...
const ComplianceScores: React.FC<ComplianceScoresProps> = ({ data }) => {
  ...
  const getStatusIcon = (score: number, target:  number) => {
    if (score >= target) return <CheckCircle className="w-5 h-5 text-green-600 dark:text-green-400" />;
    if (score >= target * 0.7) return <AlertTriangle className="w-5 h-5 text-yellow-600 dark:text-yellow-400" />;
    ...
    <div className="bg-white border border-gray-200 dark:bg-black/10 dark:border-white/10 rounded-lg p-6 h-full flex flex-col">
      ...
      <div className="space-y-4 flex-1">
        {standards.map((standard) => {
          ...
          return (
            <div key={standard.key} className="p-4 bg-gray-50 dark:bg-white/5 rounded-lg border border-gray-200 dark: border-white/5 hover:border-gray-300 dark:hover:border-white/10 transition-colors">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-start space-x-3 flex-1">
                  <div className={`p-2 rounded-lg bg-gray-100 dark:bg-white/5 ${standard.iconColor}`}>
                    <IconComponent className="w-5 h-5" />
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center space-x-2 mb-1">
                      {getStatusIcon(standard.score, standard.target)}
                      <span className="text-gray-900 dark:text-white font-semibold">{standard.name}</span>
                    </div>
                    <div className="text-gray-600 dark:text-white/60 text-xs">{standard.description}</div>
                  </div>
                </div>
      ...
      <div className="mt-6 pt-6 border-t border-gray-200 dark:border-white/20">
        <div className="grid grid-cols-3 gap-4 text-center">
          <div>
            ...
            <div className="text-gray-500 dark:text-white/60 text-xs">Compliant</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-yellow-600 dark:text-yellow-400">
              {standards.filter(s => s.score >= s.target * 0.7 && s.score < s.target).length}
            </div>
            <div className="text-gray-500 dark:text-white/60 text-xs">Near</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-red-600 dark:text-red-400">
              {standards. filter(s => s.score < s.target * 0.7).length}
            </div>
            <div className="text-gray-500 dark:text-white/60 text-xs">Non-Compliant</div>
          </div>
        </div>
      </div>
    </div>
